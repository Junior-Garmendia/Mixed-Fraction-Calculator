...
class FractionsTest
{
  ...
  void testIntPower()
  {
    ...
    // Exponent < 0(Negative)
    // (1 1/2) ^ -1 = 4/9
    Rational negAnswer = new Fraction(4, 9);
    Rational mixedOperation = mixed.integerPow(-1).format();
    assertEquals(negAnswer.format().toString(), mixedOperation.toString());

    // 2 ^-1 = 1/2
    assertEquals(new Fraction(1, 2).format().toString(), two.integerPow(-1).format().toString());
  ...
  void testWholeAndWhole()
  {
    ...
    System.out.println(new WholeNumber(4).division(new WholeNumber(3)).format().toString());
